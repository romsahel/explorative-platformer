...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    public void pushBack(bool facingRight, float force)
    {
        if (force > 1f)
            StartCoroutine(pushBackRoutine(facingRight, force));
    }

    private IEnumerator pushBackRoutine(bool facingRight, float force)
    {
        this.enabled = false;
        Vector2 dir = (facingRight) ? Vector2.left : Vector2.right;
        _rigidbody2D.AddForce(dir * 100 * force);

        do
        {
            yield return new WaitForFixedUpdate();
        } while (_rigidbody2D.velocity.x > 0.05f);
        this.enabled = true;
    }
